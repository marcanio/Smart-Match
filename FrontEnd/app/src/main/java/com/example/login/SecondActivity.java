...
public class SecondActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String lastQuiz = db.getQuiz(userLogin);
        if (lastQuiz != null && lastQuiz.length() >= 2) {
            quizResults = lastQuiz;
            endActions();
        }
        getQuestionList(db.getQuestions());
    }

    private void getQuestionList(ArrayList<String> questions) {
        questionList = new ArrayList<>();
        for (String s : questions) {
            questionList.add(new Question(s, "Disagree strongly", "Disagree a little", "Neutral", "Agree a little", "Agree strongly", 3));
        }
        setQuestion();
    }
    ...
    private void changeQuestion() {
        if (questNum < questionList.size() - 1) {
            ...
            qCount.setText(String.format("%d/%d", questNum + 1, questionList.size()));
        } else {
            //Display score- Last question
            db.updateQuizResults(userLogin, quizResults);
            endActions();
        }
    }
